...
namespace AIProj
{
  ...
  public:
     ...
     * @return
     */
    std::shared_ptr< std::set<const std::shared_ptr<Tract> > > getAvailableTracts(const districtId& dId);
		...
		const districtId& dId) const
{
	...
	{
		const std::set<tractId> &neighbors = (tractMap_[*tractSetItr])->getNeighbors();
